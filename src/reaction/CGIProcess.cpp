...
CGIProcess::CGIProcess()
    : _env(NULL), _args(NULL), _path(NULL), _pid(-1), _forwardSocket(-1),
      _inputDone(false), _childProcessDone(false)
...
std::string CGIProcess::getEnvValue(EnvMembers Member, Request &Req,
                                    Script &Script) const {
  switch (Member) {
  case SERVER_NAME:
...
bool CGIProcess::envMember(EnvMembers Index, const std::string &Key,
...
bool CGIProcess::createEnv(Request &Req, Script &Script) {
  ...
  for (int i = 0; i < NB_OF_ENV; ++i) {
    ...
    if (!envMember(member, getEnvKey(member),
                   getEnvValue(member, Req, Script))) {
      clearEnv();
      return false;
    }
  }
...
bool CGIProcess::initForwardSocket(int &ForwardSocket) {
  ...
  ForwardSocket = sv[0];
  _forwardSocket =
      sv[0]; // Adding this line to also set CGIProcess._forwardSocket
  // Networking is accessing ForwardSocket(a member of Connection that has been
  // passed through a chain of functions) However, Buffer::fileToBuf() sends to
  // _fowardSocket
  return true;
}
