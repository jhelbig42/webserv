...
void Request::parseResource(const std::string &Token) {
  ...
  logging::log(logging::Debug,
               "parse status_line: parse resource successfully");
...
void Request::parseHttp(const std::string &Token) {
  ...
  if (pos == std::string::npos || pos <= 5 ||
      pos == tback - 1) // no . or no other char before .
  ...
  logging::log(logging::Debug,
               "parse status_line: parse http version successfully");
}
...
void Request::parseRequestLine(const std::string &RequestLine) {
  ...
  if (status.size() != 3) {
    logging::log(logging::Debug,
                 "Request is invalid after split of StatusLine");
    _state = INVALID;
    return;
  }
  try {
