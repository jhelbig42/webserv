 ...
 *
 * The 2nd argument of listen() is BACKLOG, which is the max backlog of pending
 * connections. According to Beej, most sytems silently cap backlog at 20, and
 * 10 is usually sufficient.
 *
 * \param	P	pointer to an addrinfo struct
 * Must have alreadt been generated by getaddrinfo() with the the website
 ...
 * Sock must be a socket which has already been created and bound to an IP:Port
 * pair.
 */
 ...
 * Accepts a connection on a listening socket, and creates a new socket for the
 * incoming connection Throws runtime error if accept() fails, with strerror
 * message.
 *
 * \param	ListenerFd	fd of the listening socket on which to accept the
 * connection. Must be a socket which has already been created, bound to an
 * IP:Port pair, and set to listen.
 *
 * \param	Candidate	pointer to a ClientAddr struct
 * Will be filled with the new socket fd and client address info for the
 * incoming connection.
 *
